...
-- repeat purchase percentage = 0.6407 as the code showing below
SELECT 
    (SELECT COUNT(*) 
...
FROM 
    customers_dataset c;
-- when I calculate the repeat purchase revenue, it says that query is out of time, so we should optimize our query and sql server.
-- First, we try to add index to the tables
-- check if it has exist indexes

SHOW INDEX FROM customers_dataset;
SHOW INDEX FROM order_payments;
SHOW INDEX FROM orders;
 -- From the result, we found that these three table have no index. Therefore, we should try to add index to them.
 -- back up the table

-- increase wait time out and interactive time
SET @@global.wait_timeout=28800;
SET @@global.interactive_timeout=28800;

-- if the table has undergone many changes (inserts, updates, deletes), it may be fragmented. Running OPTIMIZE TABLE before adding the index 
OPTIMIZE TABLE customers_dataset;								
OPTIMIZE TABLE order_payments;
OPTIMIZE TABLE orders;
-- since we're trying to create an index on TEXT column (customer_unique_id and customer_id) without specifying a key length.
-- Therefore we can try limit the index length to the specific data needed or changing the  data type from TEXT to VARCHAR which will improve the query performance
ALTER TABLE customers_dataset MODIFY customer_id VARCHAR(255);
ALTER TABLE customers_dataset MODIFY customer_unique_id VARCHAR(255);
ALTER TABLE customers_dataset ADD INDEX idx_customer_id (customer_id);
ALTER TABLE customers_dataset ADD INDEX idx_customer_unique_id (customer_unique_id);
SHOW INDEX FROM customers_dataset;

ALTER TABLE orders MODIFY customer_id VARCHAR(255);
ALTER TABLE orders MODIFY order_id VARCHAR(255);  
ALTER TABLE orders ADD INDEX idx_customer_id (customer_id);
ALTER TABLE orders ADD INDEX idx_order_id (order_id);
SHOW INDEX FROM orders;

ALTER TABLE order_payments MODIFY order_id VARCHAR(255);
ALTER TABLE order_payments ADD INDEX idx_order_id (order_id); 
SHOW INDEX FROM order_payments;
...
JOIN order_payments op ON repeat_orders.order_id = op.order_id;


-- The repeat customers'purchasing is 0.17% of total customer purchasing, which is a extremely small value, and isn't a good indicator for customer retention.
SELECT 
    ROUND((SUM(op.payment_value) / (SELECT SUM(payment_value) FROM order_payments)) * 100, 2) AS repeat_customer_revenue_percentage
FROM 
    (
        SELECT DISTINCT o.order_id
        FROM orders o
...
-- repeate purchase customer  shopping frequency and lifetime span
    SELECT 
    customer_unique_id,
...
-- repeat purchase customers: order numbers, days interval, AOV
...
-- the number of weeks within a month sometimes is 5 sometimes is 6, 
-- It is probabily because one week across two months,so the week number appears twice.
SELECT 
...
FROM review_rate;
-- the average review score is 4.07 

-- calculate the review quantile, so that the seller can know which level they ar. 
SELECT 
...
-- the lower bond (0.25) of the review score is 4 
-- the lower bond (0.75) of the review score is 5 
...
-- optimize the buuferpool for better server performance. tying to increase mysql bufferpool size from 128M to 512M to improve the performance.
SHOW ENGINE INNODB STATUS;

SHOW GLOBAL STATUS LIKE 'Innodb_buffer_pool_pages_total';  -- 8192
SHOW GLOBAL STATUS LIKE 'Innodb_buffer_pool_pages_free'; -- 5517 

-- (1− free usage/total usage)*100 = 32.66%



SHOW GLOBAL STATUS LIKE 'innodb_buffer_pool_pages_%';
SHOW VARIABLES LIKE 'innodb_buffer_pool_size'; -- 128M, 134217728
